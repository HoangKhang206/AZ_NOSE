...
## 🎬 Demo Mode — Scope 2 ngày (QUAN TRỌNG)

Dự án hiện đang ở giai đoạn **DEMO cho thuyết trình môn CT**, KHÔNG phải production. Scope thu hẹp có chủ ý:

### Phần LÀM THẬT (đúng như spec)
- Module 1: MediaPipe Face Mesh rải 468 landmark trên ảnh user (client-side)
- Module 2: Geometric Analysis với định lý cos + Euclidean
- Module 3: AR Silhouette Overlay Cấp độ 2 (đường viền SVG, 1 template)
- Module 4 phần AI-1: RAG với Gemini 1.5 Flash + mini KB 5-10 case (thật)
- Consent đầy đủ theo Nghị định 13/2023
- Rate limiting tối giản
- Kill switch qua env vars
- Responsive mobile + desktop (2 breakpoints)

### Phần HARDCODED cho demo (chuyển sang thật ở phase 2)
- Module 4 phần AI-2 (Chatbot): 5 câu hỏi + 5 câu trả lời hardcoded
- 3 upload zone: chỉ demo với ảnh chính diện, 45°/90° chỉ là placeholder UI
- Case tương tự: dùng ảnh AI-generated placeholder
- Nút "Đặt lịch": `alert()` giả lập, KHÔNG tích hợp Google Sheets

### Phần BỎ HẲN (không làm trong demo)
- Google Sheets API integration
- GA4, Meta Pixel, Microsoft Clarity
- Multi-template AR (chỉ 1 template S-line)
- E2E testing (Playwright)
- Voice input cho chatbot
- Đa ngôn ngữ EN/KR

### Khi CLI code, cần biết
- Nếu user hỏi tính năng đã "hardcoded" → implement bản simple, KHÔNG cố gắng làm chuẩn production
- Nếu user hỏi tính năng đã "bỏ hẳn" → nhắc user đây là scope phase 2, hỏi có chắc muốn thêm không
- Biến `NEXT_PUBLIC_DEMO_MODE=true` trong .env.local để toggle logic demo/production
...
- [ ] **Có nằm trong Demo Scope (section trên) không? Nếu ngoài scope, có confirm với user chưa?**
